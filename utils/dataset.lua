...
function dataset:getByClass(class)
   ...
   local label_vec = self.Trajectory_labels[{{2*n_img-1, 2*n_img}}]
   label_vec = torch.reshape(label_vec, 1, label_vec:size()[2]*2)
   return self:sampleHookTrain(imgpath), label_vec
end

-- converts a table of samples (and corresponding labels) to a clean tensor
local function tableToOutput(self, dataTable, scalarTable, trajTable)
   local data, scalarLabels, trajLabels, labels
   ...
   return data, scalarLabels, trajLabels
end
...
function dataset:sample(quantity)
   ...
   for i=1,quantity do
      ...
      local out, tlab = self:getByClass(class)
      table.insert(dataTable, out)
      table.insert(trajTable, tlab)
      table.insert(scalarTable, class)
   end
   local data, scalarLabels, trajLabels = tableToOutput(self, dataTable, scalarTable, trajTable)
   return data, scalarLabels, trajLabels
end
